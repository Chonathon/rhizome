...
- **Placement**: on desktop with the sidebar expanded, the radio UI docks as a section above the player in the left sidebar (the "listening column"). When the sidebar is collapsed or on mobile, it falls back to a compact floating pill at top-center.
...
- `src/components/JourneyPanel.tsx` — docked sidebar section (portal into `#sidebar-radio-slot` in AppSideBar) + floating pill fallback.
